...
import (
	...
	"github.com/onlyzzg/gentol/src/gen/helper"
	"github.com/onlyzzg/gentol/src/gen/internal/generate"
	"github.com/onlyzzg/gentol/src/gen/internal/model"
	"github.com/onlyzzg/gentol/src/gen/internal/parser"
	tmpl "github.com/onlyzzg/gentol/src/gen/internal/template"
	"github.com/onlyzzg/gentol/src/gen/internal/utils/pools"
)
...
func (g *Generator) GenerateModel(schemaName, tableName string, opts ...ModelOpt) *generate.QueryStructMeta {
	return g.GenerateModelAs(schemaName, tableName, g.db.Config.NamingStrategy.SchemaName(tableName), opts...)
}

// GenerateModelAs catch table info from db, return a BaseStruct
func (g *Generator) GenerateModelAs(schemaName, tableName string, modelName string, opts ...ModelOpt) *generate.QueryStructMeta {
	meta, err := generate.GetQueryStructMeta(g.db, g.genModelConfig(schemaName, tableName, modelName, opts))
	if err != nil {
...
func (g *Generator) GenerateAllTable(opts ...ModelOpt) (tableModels []interface{}) {
	...
	schemaName := g.db.Migrator().CurrentDatabase()
	for i, tableName := range tableList {
		tableModels[i] = g.GenerateModel(schemaName, tableName, opts...)
	}
	return tableModels
}
...
func (g *Generator) genModelConfig(schemaName, tableName string, modelName string, modelOpts []ModelOpt) *model.Config {
	if modelOpts == nil {
